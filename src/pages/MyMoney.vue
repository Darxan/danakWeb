<template>
    <div v-if="isMounted">
        ...
        <section class="section_table pt-5 pb-5 flex-column">
            <div class="container ">
                <h1 class="w-100 text-center align-items-center">Mening naqd pulim</h1>
                ...
                <div class="section_table_body">
                    <table id="customers">
                        ...
                        <tr v-for="transaction in transactionList" :key="transaction.card_type.id">
                            <td>{{ transaction.card_number }}</td>
                            <td>{{ transaction.payment_time }}</td>
                            <td>{{ transaction.card_type.name }}</td>
                            <td> <img :src="getImgUrl(transaction.card_type.image)" alt="" width="40"> </td>
                            <td class="action_button_container">
                                <button><img src="@/assets/icons/edit.svg" alt=""></button>
                                <button><img src="@/assets/icons/trash-2.svg" alt=""></button>
                            </td>
                        </tr>
                    </table>
                </div>
            ...
            <div class="container">
                ...
                <div class="card_container">
                    <div class="card" v-for="card in cards" :key="card.id">
                        <div class="img_block">
                            <img :src="getImgUrl_2(card.img)" alt="">
                        </div>
...
import { axiosGet } from '@/store/axiosBase.js'
    export default {
       ...
       async mounted () {
           let resoponse = await axiosGet('/api/v1/useraccount/list/');
           if(resoponse.status == 200){
               this.isMounted = true
               console.log("resoponse.data.data", resoponse.data.data)
